...
const openRegisterModal = (register) => {
    ...
    if (isRegister) {
        ...
        document.querySelector("#title-modal-register").innerHTML = "Editar Contato";
    ...
    for (let i = 0; i < buttons.length; i++) {
        buttons[i].addEventListener('click', function () {
            ...
            for (const key in contact) {
                if (key != 'id') {
                    document.querySelector(`#form input[name="${key}"]`).value = contact[key]
                }
            }
 ...
 */
document.querySelector('.btn-open-register-modal').addEventListener('click', e => {
    formContact.reset();
